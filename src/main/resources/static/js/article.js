...
function deleteArticle() {
	...
	var url = URL + "SpringMvc/article/deleteArticle?articleId="
			+ articleId;
...
function showEditArticleDialog(id) {
	var url = URL + "SpringMvc/article/getArticle?articleId=" + id;
	$.get(url, function(data) {
		if (data.code == 1) {
...
function showArticle() {
	var url = URL + "SpringMvc/article/getArticle";
	$.get(url, function(data) {
		if (data.code == 1) {
...
function addorEditArticle() {
	...
	if (title.indexOf("修改") != -1) {
		var id = $('#id').val();
		var url = URL + "SpringMvc/article/updateArticle";
		var obj = {
			'title' : article_title,
			'link' : article_link,
			'articleId' : articleId
		...
		var url = URL + "SpringMvc/article/addArticle";
		var obj = {
			'title' : article_title,
			'link' : article_link
		};
